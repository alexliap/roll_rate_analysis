...
class SnapshotRollRateTable:
    def __init__(self, snapshot_file: str, unique_key_col: str, delinquency_col: str, obs_files: list[str], perf_files: list[str], keep_cols: list[str] = None, max_delq: int = 6,
                 detailed: bool = False, granularity: int = 1):
        ...
        if self.detailed and granularity <= 1:
            raise ValueError('Granularity should be at least equal to 2 when using detailed=True variable.')
        ...
        self.roll_rate_matrix = np.zeros([self.max_delq + 1 + self.extra_rows, self.max_delq + 1], dtype = np.int32)
    ...
    def _n_cycle_performance(self, data: pl.DataFrame, cycle: int):
        ...
        if self.detailed and cycle in [3, 4]:
            tmp = (tmp.filter(pl.col(f'obs_times_{cycle}_cycle') >= 1)
            ...
            for i in range(1, self.granularity + 1):
                idxs, values, values_plus = self._detailed_delinquencies(data=tmp, cycle=cycle, rank=i)

                self._update_matrix(cycle=cycle, idxs=idxs, values=values, plus_values=values_plus, rank=i)

        else:
            tmp = (tmp.groupby(['obs_max_delq', 'perf_max_delq'])
                   .count()
                   .sort('perf_max_delq'))
    ...
    def build(self):
        ...
        if self.detailed:
            result = result.select([self.unique_key_col, 'obs_max_delq', 'obs_times_3_cycle', 'obs_times_4_cycle', 'perf_max_delq'])
        else:
    ...
    def _build_obs_part(self):
        ...
        if self.detailed:
            ...
            self.obs_data = self.obs_data.with_columns([pl.sum_horizontal(cs.starts_with(self.delinquency_col) == 3).alias('obs_times_3_cycle'),
                                                        pl.sum_horizontal(cs.starts_with(self.delinquency_col) == 4).alias('obs_times_4_cycle')])
    ...
    def _detailed_delinquencies(self, data: pl.DataFrame, cycle: int, rank: int):
        if rank != self.granularity:
            idxs = data.filter((pl.col(f'obs_times_{cycle}_cycle') == rank) & (pl.col('perf_max_delq') < self.max_delq))['perf_max_delq'].to_numpy()
            values = data.filter((pl.col(f'obs_times_{cycle}_cycle') == rank) & (pl.col('perf_max_delq') < self.max_delq))['count'].to_numpy()
            values_plus = data.filter((pl.col(f'obs_times_{cycle}_cycle') == rank) & (pl.col('perf_max_delq') >= self.max_delq))['count'].sum()
        else:
            data = data.filter(pl.col(f'obs_times_{cycle}_cycle') >= rank).select(['perf_max_delq', 'count']).group_by(['perf_max_delq']).sum()

            idxs = data.filter((pl.col('perf_max_delq') < self.max_delq))['perf_max_delq'].to_numpy()
            values = data.filter((pl.col('perf_max_delq') < self.max_delq))['count'].to_numpy()
            values_plus = data.filter((pl.col('perf_max_delq') >= self.max_delq))['count'].sum()
    ...
    def _update_matrix(self, cycle, idxs, values, plus_values, rank: int = 1):
        ...
        if cycle == 4:
            self.roll_rate_matrix[cycle - 1 + self.granularity + rank - 1, idxs] += values
            self.roll_rate_matrix[cycle - 1 + self.granularity + rank - 1, self.max_delq] += plus_values
        elif cycle == 5:
            ...
            self.roll_rate_matrix[self.max_delq + self.extra_rows, self.max_delq] += plus_values

    def null_to_zero(self, data: pl.DataFrame):
        for column in data.columns[1:]:
            data = data.with_columns(pl.when(pl.col(column).is_null())
                                     .then(pl.lit(0))
